...
export default function PlanCard({ plan }) {
  ...
  const handleNextOption = () => {
    setSelectedOptionIndex((prev) =>
      prev < plan.options.length - 1 ? prev + 1 : prev
    );
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-center mb-6">
        <h2 className="lg:text-4xl text-2xl text-nowrap font-bold">
          {plan.name} Plan
        </h2>
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between">
          ...
          <span className="lg:text-xl text-lg font-semibold">
            {selectedOption.rangeOfUsers} users
          </span>
          <button
            onClick={handleNextOption}
            disabled={selectedOptionIndex === plan.options.length - 1}
            className="p-2 rounded-full bg-gradient-to-r from-orange-500 to-orange-600 text-white disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-100"
          >
